 ...
 * Description:       Replaces the WordPress login flow with a custom page.
 * Version:           1.0.0
 * Author:            Jason Aston
 * License:           GPL-2.0+
...
class Personalize_Login_Plugin {
     ...
     * hooks in the constructor.
		*remove_filter( 'authenticate', 'wp_authenticate_username_password', 20 ); to use as email as authentication  - add beforeauthenticate
 ...
 */
	...
	public function render_login_form( $attributes, $content = null ) {
		...
		$attributes['lost_password_sent'] = isset( $_REQUEST['checkemail'] );
 ...
 */
	...
	public function do_register_user() {
	    if ( 'POST' == $_SERVER['REQUEST_METHOD'] ) {
	        ...
	        if ( ! get_option( 'users_can_register' ) ) {
			    ...
			    $result = $this->register_user( $email, $display_name, $first_name, $last_name );
	...
	/**
	* Redirect the user to the custom login if they are not logged in
	* and viewing a member account page.
	*/

	public function redirect_to_protect_account_pages() {
	if ( ! is_user_logged_in() && is_page( array( 'member-account' ) ) ) {
