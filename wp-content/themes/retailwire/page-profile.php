...
if ( 'POST' == $_SERVER['REQUEST_METHOD'] && !empty( $_POST['action'] ) && $_POST['action'] == 'update-user' ) {
    ...
    if ( !empty( $_POST['display-name'] ) )
        update_user_meta( $current_user->ID, 'display_name', esc_attr( $_POST['display-name'] ) );
    if ( !empty( $_POST['twitter'] ) )
        wp_update_user( array( 'ID' => $current_user->ID, 'twitter' => esc_url( $_POST['twitter'] ) ) );
    if ( !empty( $_POST['facebook'] ) )
        wp_update_user( array( 'ID' => $current_user->ID, 'facebook' => esc_url( $_POST['facebook'] ) ) );
    if ( !empty( $_POST['linked-in'] ) )
        wp_update_user( array( 'ID' => $current_user->ID, 'linked_in' => esc_url( $_POST['linked-in'] ) ) );
    if ( !empty( $_POST['display-name'] ) )
        ...
        <div class="entry-content entry">
            ...
            <?php else : ?>
                ...
                <form method="post" id="adduser" action="<?php the_permalink(); ?>">
                    ...
                    <p class="form-linked-in">
                        <label for="linked-in"><?php _e('Linked In', 'profile'); ?></label>
                        <input class="text-input" name="linked-in" type="text" id="linked-in" value="<?php the_author_meta( 'linked_in', $current_user->ID ); ?>" />
                    </p><!-- .form-url -->
                    
                   <p class="form-position">
                        <label for="position"><?php _e('Position', 'profile') ?></label>
                        <textarea name="position" id="position" rows="1" cols="50"><?php the_author_meta( 'position', $current_user->ID ); ?></textarea>
                    </p><!-- .form-textarea -->
